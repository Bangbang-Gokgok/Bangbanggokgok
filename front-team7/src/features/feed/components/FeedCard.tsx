...
import { FeedBottom } from '@/features/feed/components';

interface FeedCardProps {
  feed: Partial<FeedsResponse>;
}

export const FeedCard = ({ feed }: FeedCardProps) => {
  const { userId, title, description, address, location, imageUrl, createdAt } = feed;

  const image = imageUrl![0] ?? noImage;

  return (
    <StyledFeedCard image={image}>
      <Link
...
const StyledFeedCard = styled.article<{ image: string }>`
  border-radius: 8px;
